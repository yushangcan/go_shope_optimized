...
本目录对应 `codex/optimized-seckill`，用于和普通版 `baseline-v1.0.0` 做同条件单机对比。优化版的秒杀接口只负责快速受理：请求先由 Redis Lua 原子校验并扣减预热库存，再写入 Redis Stream；单个 Worker 异步执行现有 MySQL 事务，最终状态通过查询接口确认。
